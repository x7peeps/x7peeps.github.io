...
- `日志清理与反取证痕迹识别`：提供 Rootkit 清理后的旁证恢复方法

本文的定位是聚焦 `0x02` 病毒查杀检查中"多引擎判定差异"和"Rootkit 残留检测"这两个维度，而不是覆盖整个恶意软件检测领域。

---

## 0x0A 总结

病毒查杀和 Rootkit 检测分析的关键，不是"跑了哪些工具"，而是：

- 理解不同工具的检测维度和覆盖范围差异
- 正确解读判定差异，区分误报和真实威胁
- 通过多工具交叉检测降低漏报风险
- 通过进程/文件隐藏检测发现 Rootkit 残留

当你能从 ClamAV、chkrootkit、rkhunter 的判定结果中读出检测差异、误报信号、Rootkit 残留线索时，`0x02` 里的"病毒查杀检查"才真正升级为 `0x03` 的"病毒查杀检查结果与多引擎判定差异及 Rootkit 残留分析"。
