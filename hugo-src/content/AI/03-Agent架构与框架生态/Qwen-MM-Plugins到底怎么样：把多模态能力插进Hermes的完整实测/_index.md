...
Qwen-MM-Plugins 是阿里官方的多模态能力插件框架：把读图、读视频、读 PDF 等能力做成可插拔工具，让任何 AI 框架都能接入。本文用 9 类真实文件、14 个场景实测它接入 Hermes 后能提升什么、提升多少。

核心结论：

1. **看图从"多走两步"变"一步到位"**。文本模型本身没有视觉，无桥接时要么看不见，要么手动调工具多花 26-40 秒、还可能偶发失败；桥接后图片自动附带文字描述，直接回答。
2. **多类型文件从"看不了"变"能看"**。视频自动抽帧、PDF 直接读全文、表格/代码/3D 能可视化——这是纯文本模型之前做不到的。
...
- **Qwen-MM-Plugins**：一套「文件处理工具 + 说明书」——负责读图、抽视频帧、翻 PDF、查文件信息这些**处理动作**，并告诉 AI 怎么用。它的工具链执行到最后一步**会调用视觉模型做理解**，但工具链本身提供的是「怎么读、读多少、按什么顺序」的流程和格式转换，不是一个模型——像「手 + 工具箱 + 说明书」
- **主模型**（比如 deepseek）：**纯文本模型，永远收不到图片像素**——它拿到的是视觉模型翻译好的文字描述，再思考分析——像"大脑"（但不长眼睛，靠别人描述）

所以它更准确的定位是：**AI 的"感官扩展包"**。

注意：Hermes 本身就能调用视觉模型看图（内置 vision_analyze 工具），所以「加不加以后能不能看」不是区别——真正的区别是**手动 vs 自动、单图 vs 全类型、拼命令 vs 标准工具链**：

| 给 AI 发来什么 | Hermes 原本（手动调工具） | 加了 Qwen-MM-Plugins |
|---|---|---|
| 一张截图 | AI 要「想起来」看不见 → 手动调 vision_analyze → 等 26-40 秒 → 再回答 | 图片一到**自动**出摘要 → 直接回答（约 4 秒） |
| 一段视频 | 手动 ffmpeg 抽帧 → 逐帧调 vision_analyze（多次往返） | read_video **一键抽帧** → 自动逐帧描述 |
| 一份 PDF 报告 | read_file 读文本层（扫描件/图片型 PDF 读不出） | visualize 渲染页面 + 提取文字 |
| Excel/CSV 表格 | 结构化工具能读数字，但图表形态/趋势看不清 | 自动渲染成图表理解 |
| 代码/3D 文件 | 文本代码能读，3D/地理文件打不开 | 能可视化 |

所以准确的说法是：**Hermes 原本「能做但费劲」（手动、单图、拼命令），加了 Qwen-MM-Plugins 变成「自动、全类型、标准流程」**。
...
回答"提升多少"，关键是**控制变量**。文本模型（deepseek）本身没有任何视觉能力，所以"它能不能看图"取决于系统给了它什么视觉通道。这里必须区分两件事：

- **无视觉通道**：模型只拿到 `MEDIA:<路径>` 文本，且不允许调用任何工具——它真的看不见。
- **有视觉通道但无桥接**：模型拿到路径后，**可以主动调用 vision_analyze 工具**——它能看见，但要"想起来"、多一次工具往返、多等一次视觉推理。
- **有桥接**：路径 + 自动摘要一次到位——直接回答。

所以严谨的对照是三组，而不是两组：

| 组 | 输入/通道 | 模拟的真实系统状态 |
|---|---|---|
| 组1 | `MEDIA:<路径>`，禁调工具 | 无任何视觉通道的文本模型 |
| 组2 | `MEDIA:<路径>` + 可调 vision_analyze | 真实 Hermes 无桥接（agent 可手动调工具） |
| 组3 | `MEDIA:<路径>` + 自动摘要 | 真实 Hermes 有桥接 |

组1 对比组3 说明"视觉通道的有无"；**组2 对比组3 才是桥接的真实增量**——同样的视觉信息都能拿到，桥接省的是：工具往返次数（1→0）、端到端延迟、以及偶发失败率。
...
4. **上限未测到**。为验证"最慢能到多少"，我构造了两张极限图：2000×2800 的超大密集表格（684KB，80 行×8 列）耗时 104 秒；1800×6000 的超长文本页（5.2MB）反而只要 31.9 秒——说明延迟主要取决于**内容密度与输出长度**而非单纯的文件大小。更极端的输入（更高分辨率、更复杂版面）延迟会继续上升，本文未测到绝对上限。
...
先用三组对照把变量控制清楚（见 5.1）。核心对比是**组2 vs 组3**：两者都能拿到视觉信息，桥接的真实增量是效率与可靠性。

| 场景 | 组1 无视觉通道 | 组2 无桥接（手动调 vision_analyze） | 组3 桥接 |
|---|---|---|---|
| K 线图 | 无法回答 | 40.6s · 1次调用 · 答对 | **3.8s · 0次调用 · 答对** |
| 财务 OCR | 无法回答 | 26.3s · 1次调用 · **偶发空响应** | **3.9s · 0次调用 · 数字全对** |

组2 的花费都在"模型想起来调工具 → 等视觉模型推理（20-36 秒）→ 再组织回答"；组3 的视觉推理发生在工具返回时（摘要预先附带），主模型拿到直接回答。更关键的是组2 的偶发失败：财务 OCR 场景里，手动调 vision_analyze 后 deepseek 返回了空响应——多一跳就多一个失败点。

下面按场景看回答质量对比（组1 vs 组3，同一主模型、同一问题、同一温度，唯一变量是输入里有没有摘要）。
