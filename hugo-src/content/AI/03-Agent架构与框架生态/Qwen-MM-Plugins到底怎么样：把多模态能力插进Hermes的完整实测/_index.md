...
**问：你说 100 行代码让文本模型"看得见"，是不是夸大了？换个说法，这 100 行到底补了什么？**

没有夸大，但要精确理解"补了什么"。Hermes 的 MCP 图片链路里，缺的从来不是"读图能力"——qwen-mm-plugins 会读、Mage-VL 会读、Hermes 的 vision_analyze 也会读。缺的是一个**自动触发点**：图片到达系统后，系统不知道"这个文本模型看不见，得先翻译一下"。桥接就是在 MCP 结果处理层加了这一个触发点。它不创造能力，它接通能力。这也是为什么信息增益能到 23.5 倍——因为之前那 66 个字符的元数据对推理贡献几乎为零，而摘要直接给了可用的信息。

**问：为什么不用多模态主模型，一劳永逸？**

多模态主模型（GPT-4o、Qwen-VL 这类）确实原生能看图，延迟还低。但代价是：贵（多模态输入 token 计价高）、依赖云端（隐私场景不可用）、模型选择受限（不是所有好用的文本模型都有多模态版）。deepseek-v4-flash 这种便宜高效的文本模型 + 本地视觉模型的组合，在成本和隐私上优势明显。两条路线不冲突：预算充足、隐私不敏感的场景上多模态主模型；成本敏感、隐私敏感、需要本地化的场景用桥接。

**问：摘要延迟 30 秒，用户等得起吗？**

等不起，所以我把默认超时设成了 20 秒，并且 `mcp_summary_timeout` 可调。真实使用中这是一个权衡：摘要质量 vs 等待时间。我的实测里，50KB 左右的图 8-11 秒，22KB 视频帧 5.3 秒——日常截图级别是可接受的；183KB 的整页截图 31 秒就偏慢了，这种场景应该先缩小图或者直接让视觉模型看缩略图。优化方向明确：降分辨率 + 换快模型 + 并行。

**问：如果视觉模型把图片里的恶意指令读出来怎么办？**

这是真实的注入面，我在 8.4 节详细分析了。缓解的核心是"标注来源 + 角色隔离"：摘要带 `[图片内容摘要]` 前缀，主模型把它当数据不当指令。这和人读网页遇到钓鱼内容的逻辑一样——内容可以有毒，但执行权在模型手里。高安全场景可以关掉桥接，代价是回到"看不见"。

**问：你测的素材都是自己生成的，真实世界效果会打折吗？**

会打折，所以要诚实。我的 K 线图是 matplotlib 生成的（干净、标注清晰），财务图是 PIL 渲染的文字（无噪点、无遮挡），这都比真实世界理想。真实世界的图有反光、模糊、遮挡、低对比，视觉模型准确率会下降。但注意：**本文的对照组实验恰恰不依赖视觉模型的绝对准确率**——基线组是"完全看不见"，实验组是"大部分能看见"。即使视觉模型偶尔认错，信息可用性也是从 0 到远大于 0 的质变。真实世界效果的打折体现在摘要质量，不体现在"有没有摘要"。

**问：Qwen-MM-Plugins 和 Hermes 自己的 skill 体系重复吗？**

不重复，是互补。Hermes 的 skill 是"教模型怎么做"，Qwen-MM-Plugins 的 capability 是"给模型干活的工具 + 使用说明"。接入后，Qwen-MM-Plugins 的 SKILL.md 变成 Hermes 技能库里的一枚技能，它的 MCP 工具注册进 Hermes 的工具集，两者通过 skill 描述 + MCP schema 协同。这其实验证了 Hermes"扩展优先、能力在边缘"的架构哲学——新能力不需要改核心，插件化接入即可。

**问：接入后 Hermes 的日常体验到底变化了什么？说一个最直观的。**

最直观的变化是：**过去需要我"想起来"的事，现在系统自动做了**。以前用户发一张截图说"看看这个"，我要先意识到"我是文本模型、我看不见"，然后显式调 vision_analyze 工具，等它描述完再推理。现在 MCP 工具返回图片的瞬间，摘要已经躺在工具结果里了，我直接就能回答。这个变化不是"更快了一点"，是"从两段式变成一段式"——少了一次认知开销、少了一次工具调用的往返、少了一段上下文切换。对 agent 来说，这种"不需要想起来"的自动化，价值比延迟数字大得多。

**问：视频场景 10 帧每帧都摘要，实际会怎么处理？**

当前实现是每帧独立触发（有会话缓存，同帧不重复）。10 帧 = 10 次推理 ≈ 50 秒，这在真实场景不可接受。合理策略是：a) 只摘要关键帧（抽帧工具先按场景切分，选代表帧）；b) 并行摘要（多帧同时推理）；c) 视频场景单独配置更小的 budget（小图快很多）。这些是桥接的下一步优化，不是当前版本的缺陷——当前版本的定位是"图片场景先跑通"，视频是明确标注的边界。

**问：你说摘要可能带幻觉，那 OCR 数字为什么可信？**

OCR 场景的数字可信，是因为我做了**交叉验证**：Mage-VL 把数字读出来（"45.6 亿元，同比增长 23.4%"），deepseek 整理时数字一致（"预计上半年营业收入：45.6 亿元，同比增长 23.4%"）——两个独立模型对同一组数字的解读吻合，错误概率大幅下降。单模型单次输出永远有幻觉风险，但多模型交叉 + 关键数据人工复核，能把风险压到可接受范围。这也是我对文章里所有"正确回答"结论的验证方式。
