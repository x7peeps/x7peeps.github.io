...
1. **发一张截图，AI 不再是"我看不见"，而是直接回答内容**。之前把网页截图、聊天记录、报错界面发给文本模型，它只能回"我无法查看图片"；接入后，同一张图到了 AI 手里会自动附带视觉描述——网页能说清是什么网站、有什么栏目，聊天记录能总结谁说了什么，报错截图能直接给出"余额不足，检查下单前扣款逻辑"这样的修复建议。
