...
1. **发一张截图，AI 从"多走两步"变成"直接回答"**。文本模型本身没有视觉，无桥接时它要么只能回"我无法查看图片"（无视觉通道可用），要么得自己想起来去调 vision_analyze 工具——实测这要多花 26-40 秒、多一次工具往返，还可能偶发失败；接入桥接后，图片一到 AI 手里就自动附带视觉描述，直接回答——网页能说清是什么网站、有什么栏目，聊天记录能总结谁说了什么，报错截图能直接给出"余额不足，检查下单前扣款逻辑"这样的修复建议。
2. **数字从图里自动提取，一个不漏**。财务报告截图、表格截图里的关键数字（营收 45.6 亿、净利润 8.2 亿、同比增长 23.4%……）能被自动读出来并整理成结构化答案，实测 6 个关键数字全部一致。手写会议纪要也能识别成可检索文本。
3. **图表能被"读懂"而不只是"看到"**。K 线图能识别出是哪只股票（贵州茅台）、什么周期（60 日线）、什么形态（先扬后抑再回升）；柱状图、折线图、饼图组合的经营分析报告，能读出每个图的数据和含义。
4. **视频和 PDF 从"看不了"变成"能看"**。视频自动抽帧、逐帧描述内容（人物、场景、字幕）；PDF 直接提取全文文字。这些在接入前是文本模型完全做不到的。
5. **这一切是自动发生的**。不需要 AI"想起来"自己看不见、手动去调工具——图片一到就自动翻译成文字描述，AI 直接拿着描述继续推理。

代价是延迟：本地视觉模型单图分析 5-72 秒，复杂图（表格、整页截图）最慢，这是当前方案的主要成本。能力边界也清晰：PDF 文本提取不需要桥接，视频多帧需要采样策略控制成本。
...
|---|---|---|
| 一张截图 | AI 只能看到一行路径，读不出内容 | 视觉模型自动翻译成文字，AI 能读懂内容 |
| 一段视频 | 看不了 | 自动抽帧，视觉模型逐帧描述 |
| 一份 PDF 报告 | 看不了 | 直接提取全文文字 |
| 一个 Excel/CSV 表格 | 看不了 | 自动渲染成图表理解 |
| 代码文件 / 3D 模型 | 看不了 | 能可视化 |

一句话：不加它，AI 只有一双"手动眼睛"（得想起来才调视觉模型），只能看照片；加了它，AI 有了一双"自动眼睛"——注意这里的"看"都是指**视觉模型把内容翻译成文字、文本主模型读取文字**，主模型本身永远不接触像素。照片、视频、文档、表格、代码全能"看"，**从"只能看照片"变成"能看整个世界"**。它的价值不在"看得更准"（那是视觉模型的事），而在"让 AI 能感知所有类型的文件，并且是自动的"。
