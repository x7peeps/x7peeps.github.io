...
**导读**：这是一篇关于"怎么科学地评测本地模型"的方法论文章，用信息梯度（Information Gradient）和决策归因（Decision Attribution）两个维度量化"模型能否自己想到正确方法"，在两个本地模型（35B MoE 与 27B chat）上完成了 40 次跑通的实测，输出包含实验设计、完整数据表和三个不避开就白测的方法论陷阱。
