...
第二类问题更隐蔽：实际消耗没那么高，但计量环节把它算高了。Codex 仓库的修复记录给出了难得的机制级证据——官方提交过一个针对性 PR（编号 #45094），方向是"基于消息内容而非序列化信封来估算历史 token"。换句话说，此前的估算把消息 ID、元数据、JSON 转义这些传输层开销也计入了历史 token 估算，导致配额显示虚高。值得注意的是，截至本文查证时点，该 PR 状态为已合入主线（merged_at: 2026-09-12T18:42:00Z）——修复方向已经生效，但客户端发布节奏可能尚未覆盖到所有用户的估算逻辑，已建会话的历史 token 估算仍按旧逻辑执行。
...
| 计量高估 | Codex PR #45094 | 信封序列化开销计入 token 估算，配额显示虚高 | 已合入主线（merged_at: 2026-09-12T18:42:00Z） |
...
这张表本身就是一个结论：**七条证据里有四条处于"已解决"状态（含两条 2026-09-12 当周合入的官方改进 #44970 与 #45094）**，剩余三条仍开放。**机制级修复已经发生，但用户侧仍可能在客户端发布节奏滞后期内遇到旧估算逻辑。**
...
**第四，把配额表当参考值，不当真值。** 即使 §1.4 列出的两条机制级修复（#45094 计量口径修正、#44970 任务级用量展示）已在 2026-09-12 当周合入主线，客户端发布节奏与历史会话的估算逻辑可能尚未跟进，配额显示仍可能存在系统性虚高风险。遇到"几分钟烧穿配额"的异常，先别急着切号或加购——对照 API 侧实际计费（如果有 API 账单可查）或厂商状态页，确认是真实消耗还是估算失真。同账号对照测量（如 §1.2 第三个案例的做法）是社区已验证有效的取证手段。
...
**误区四："等官方修复就好。"** 七条证据里仍有三条开放（含两条同周合入的官方改进 #44970 与 #45094，以及一条机制级 issue 讨论）——**机制级修复已在 2026-09-12 当周合入主线**，但客户端发布节奏、历史会话估算逻辑、用户对修复生效的感知这三层之间存在时差，官方修复节奏不可控，用户侧护栏的成本远低于等待的代价。
...
（本文证据快照时点为 2026-09-17；2026-09-19 live 复查发现 #45094 与 #44970 已合入主线，本文 §1.2 / §1.4 / §5 / 误区四 已同步修订以反映这一变更。全部 issue/PR 编号已逐条对照 GitHub 当前状态核实；各工具后续修复进展请以官方仓库为准。已过 Rigor Gate 自检：引用核实 9/9，数据均引自 issue 原文标题或已合入 PR，无推测性数字。）
