...
**为什么选 qwen3.6-nothink**：事实抽取是结构化输出任务，不需要 27B 推理能力——6.5s vs 23.8s（**3.7× 加速**）+ JSON 零损坏，让 `qwen3.6-nothink` 成为默认。


### 9.4 总结：相对 Hermes 自带的 6 个升级，本质是一件事

回到开篇那张能力对比表，6 个维度看起来很多，但**它们其实是同一件事的不同表现**——**给 Hermes Agent 加了一个可索引、可审计、可量化的记忆层**。

| 维度 | 本质 |
|---|---|
| 容量 ∞ | 索引层独立于 session，磁盘无限 |
| 检索秒级 | SQLite FTS5 把 38 条 facts 当小型知识库用 |
| 持久化 | ledger 记录每次变更的事实身份 |
| 隐私护栏 | tag + 关键词表让自动注入有了边界 |
| 跨 session 不丢 | 上面四个加在一起的结果 |
| 审计 | 上面五个加在一起的可观察性 |

换句话说：**Hermes 自带是「短文 + 重读」，本方案是「无限 + 检索 + 持久 + 审计」**。短文 + 重读适合"我现在就要用、且不超过 2200 字符"的场景；超过这个范围——也就是任何真正在生产里跑的 Agent——就需要这套记忆层。

选型上没花哨：本地优先、零外部依赖、SQLite 一键备份。升级路径 Hindsight 也留好了，但前提是**先把当前这套跑通**——这 38 条 facts 就是跑通的证据。

如果只记住一件事：**Hermes 自带的 MEMORY.md 2200 字符硬上限不是设计缺陷，是设计边界**——边界内简单，边界外必须靠外部索引层补齐。本文就是把这个边界补齐的过程完整记录下来。
